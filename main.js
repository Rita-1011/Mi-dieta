...
// Both the edge function and applyNormalizationRenames enforce that canonical
// names must already exist in the input — no new names are ever invented.
async function semanticNormalizeShopping(merged) {
  ...
  try {
    ...
    return applyNormalizationRenames(merged, data.merges);
  } catch {
    return merged;
  }
}
